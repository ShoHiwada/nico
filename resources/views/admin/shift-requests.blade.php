...
<!-- 締切設定フォーム -->
<div class="mb-6 border rounded shadow p-4 bg-gray-50">
    <h3 class="font-semibold text-lg mb-3">シフト希望の締切設定</h3>

    <form id="deadlineForm" action="{{ route('admin.deadlines.store') }}" method="POST"
        class="flex flex-col sm:flex-row sm:items-center sm:gap-4 gap-3">
        @csrf

        <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
            <label class="text-sm shrink-0">対象月：</label>
            <select id="selectedMonth" name="month"
                class="border rounded px-3 py-1 text-sm w-full sm:w-[140px]">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
            <label class="text-sm shrink-0">締切日：</label>
            <input type="date" name="deadline_date"
                class="border rounded px-3 py-1 text-sm w-full sm:w-[160px]" required>
        </div>

        <button type="submit"
            class="bg-blue-500 text-white px-5 py-1 rounded text-sm w-full sm:w-[100px]">保存</button>

        <div id="existingDeadline" class="text-sm text-gray-500 sm:ml-4"></div>
    </form>
</div>
...
<form method="GET" class="mb-4">
    ...
    <select name="month" onchange="this.form.submit()"
        class="border rounded px-3 py-1 text-sm w-[160px] sm:w-[140px]">
        @foreach ($availableMonths as $month)
